...
public class Enemy {
	...
	public Enemy(int type, int x, int y, float width, float height, float g, boolean fly, int lvl, boolean isboss) {
		...
		level = lvl;
		/*
		 * if(type==1) //non moving boss walk=false;
		 */
		spawnTime = 0.60f;
		...
		movevel = (float) (EnemyConst.Velv[type] + Assetloader.getrandom(1+(int) (EnemyConst.Velv[type] * 0.5f)));
		attackdelay = 0;
		State = state.idle;
		isAlive = true;
		this.width = width;
	...
	public void update(float delta, Player p1, GameWorld world) {
		...
		if (dialogtime > 0) {
			...
			case 0:
				world.enmage(magespec(1000));
				break;
			case 1:
				world.DropBarrage(new barrage(p1.getX(), 10, (int) p1.getwidth(), 60, att * 4, 1));
				break;
		...
		/*
		 * if(near(p1)) { if(State==state.idle) attack(p1); } else if(State==state.idle && distance<alertrange ) { move(p1.getX(),p1.getY()); //System.out.println(type+"moving"); } else {
		 * State=state.idle; //System.out.println(type+"idle"); }
		 */
		if (attackdelay > -1)
			attackdelay -= delta;
		...
		if (State == state.attack) {
			...
			width = fixwidth;
			/*
			 * for(boolean b:shot) b=false; timer=0; basic.delay-=delta;
			 */
		}
		if (State == state.idle) {
			if (walk && !isAlert(p1)) {
				...
				/*
				 * if(pos.x>p1.getX() && pos.x < p1.getX()+p1.rect.width) vel.x=0;
				 */
				pos.x += vel.x * delta;
	...
	public TextureRegion currentframe(float delta) {
		switch (State) {
		case attack:
			return (Assetloader.enemyattackA[type - minionoffset].getKeyFrame(timer));
		case idle:
			if (vel.x != 0)
				return (Assetloader.enemywalkA[type - minionoffset].getKeyFrame(delta));
			else
				return (Assetloader.enemystandA[type - minionoffset].getKeyFrame(delta));
		case hit:
			return (Assetloader.enemyhitA[type - minionoffset].getKeyFrame(hittime));
		default:
			return (Assetloader.enemystandA[type - minionoffset].getKeyFrame(delta));
		}
	}
	...
	public bullet magespec(int damage) {
		// TODO Auto-generated method stub
		bullet e = new bullet(1, (pos.x + width / 2) + (isleft ? 20 : -20), ground + height - 120, 120, 120, damage, !isleft);
		return e;
	}

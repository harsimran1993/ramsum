...
public class GameRenderer {
	...
	private void drawGrass() {
		...
		if (Myworld.level.water) {
			...
			batcher.draw(Assetloader.grass[Myworld.level.grass], waterf.getX(), waterf.getY() - waterf.getHeight(), waterf.getWidth(), waterf.getHeight() * 2);
			// Assetloader.font.draw(batcher, "front",
			// frontGrass.getX(),frontGrass.getY());
			batcher.draw(Assetloader.grass[Myworld.level.grass], waterb.getX(), waterb.getY() - waterb.getHeight(), waterb.getWidth(), waterb.getHeight() * 2);
			// Assetloader.font.draw(batcher, "back",
			// backGrass.getX(),backGrass.getY());
			batcher.draw(Assetloader.grass[Myworld.level.grass], waterb2.getX(), waterb2.getY() - waterb2.getHeight(), waterb2.getWidth(), waterb2.getHeight() * 2);
			// layer bottom
			batcher.setColor(batcherclr.r, batcherclr.g, batcherclr.b, 0.9f);
			batcher.draw(Assetloader.grass[Myworld.level.grass], frontGrass.getX(), frontGrass.getY() - frontGrass.getHeight() / 2, frontGrass.getWidth(), frontGrass.getHeight() * 1.5f);
			// Assetloader.font.draw(batcher, "front",
			// frontGrass.getX(),frontGrass.getY());
			batcher.draw(Assetloader.grass[Myworld.level.grass], backGrass.getX(), backGrass.getY() - frontGrass.getHeight() / 2, backGrass.getWidth(), backGrass.getHeight() * 1.5f);
			// Assetloader.font.draw(batcher, "back",
			// backGrass.getX(),backGrass.getY());
			batcher.draw(Assetloader.grass[Myworld.level.grass], backGrass2.getX(), backGrass2.getY() - frontGrass.getHeight() / 2, backGrass2.getWidth(), backGrass2.getHeight() * 1.5f);

			batcher.setColor(batcherclr.r, batcherclr.g, batcherclr.b, 1f);
		} else {
			batcher.draw(Assetloader.grass[Myworld.level.grass], frontGrass.getX(), frontGrass.getY(), frontGrass.getWidth(), frontGrass.getHeight());
			// Assetloader.font.draw(batcher, "front",
			// frontGrass.getX(),frontGrass.getY());
			batcher.draw(Assetloader.grass[Myworld.level.grass], backGrass.getX(), backGrass.getY(), backGrass.getWidth(), backGrass.getHeight());
			// Assetloader.font.draw(batcher, "back",
			// backGrass.getX(),backGrass.getY());
			batcher.draw(Assetloader.grass[Myworld.level.grass], backGrass2.getX(), backGrass2.getY(), backGrass2.getWidth(), backGrass2.getHeight());
			// Assetloader.font.draw(batcher, "back2",
			// backGrass2.getX(),backGrass2.getY()); if(Myworld.level.grass ==
			// 2)
		}
		if (Myworld.level.upwall) {
			batcher.draw(Assetloader.upwall[Myworld.level.upwallid], frontGrass.getX(), 0, frontGrass.getWidth(), frontGrass.getHeight() * 1.5f);
			// Assetloader.font.draw(batcher, "front",
			// frontGrass.getX(),frontGrass.getY());
			batcher.draw(Assetloader.upwall[Myworld.level.upwallid], backGrass.getX(), 0, backGrass.getWidth(), backGrass.getHeight() * 1.5f);
			// Assetloader.font.draw(batcher, "back",
			// backGrass.getX(),backGrass.getY());
			batcher.draw(Assetloader.upwall[Myworld.level.upwallid], backGrass2.getX(), 0, backGrass2.getWidth(), backGrass2.getHeight() * 1.5f);
		}
	}
	...
	public void render(float runTime, float delta) {
		...
		if (Myworld.isPause()) {
			Assetloader.font.setScale(0.7f, -0.7f);
			Assetloader.font.setColor(Color.YELLOW);
			batcher.begin();
			Myworld.inventory.render(batcher, gameWidth, gameHeight);
			batcher.end();
		} else
			renderPlay(runTime, delta);

	}

	public void renderPlay(float runTime, float delta) {
		// TODO Auto-generated method stub
		...
		batcher.begin();
		// background
		batcher.draw(Assetloader.cloud[Myworld.level.back], bg1.getX(), bg1.getY(), bg1.getWidth(), bg1.getHeight());
		batcher.draw(Assetloader.cloud[Myworld.level.back], bg2.getX(), bg2.getY(), bg2.getWidth(), bg2.getHeight());
		...
		/*
		 * if(Myworld.ismuLt()) { batcher.draw(Myworld.getopponent().currentframe(runTime), Myworld.getopponent ().getX(),Myworld.getopponent().getY(),Myworld.getopponent
		 * ().getwidth(),Myworld.getopponent().getheight()); Assetloader.font.draw(batcher, ""+Myworld.getopponent().name, Myworld.getopponent().getX()+(Myworld.getopponent().isleft?-45:35),
		 * Myworld.getopponent().getY()-20); Assetloader.font.draw(batcher, ""+Myworld.getopponent().hp, Myworld.getopponent().getX()+(Myworld.getopponent().isleft?-50:30),
		 * Myworld.getopponent().getY()-40); if(Myworld.getopponent().ishurt() && Myworld.getopponent().hurttime<1.2f) { Assetloader.font.setColor(Color.YELLOW); Assetloader.font.setScale(1.2f,1.2f);
		 ...
		 * Myworld.getopponent().hurttime+=delta; Assetloader.font.setColor(Color.WHITE); Assetloader.font.setScale(1f,1f); } else if(Myworld.getopponent().ishurt()) {
		 * Myworld.getopponent().hurtnomore(); }
		 * 
		 * } else
		 */
		for (Enemy enemy : en) {
			// renderdamage point above enemy while hurt else put enemy out of
			// hurt stage
			...
			if (enemy.isOnScreen()) {
				if (enemy.isAlive) {
					...
					batcher.draw(Assetloader.specialeffA[3].getKeyFrame(enemy.spawnTime), enemy.getX() - 20, enemy.getY() - 10, (enemy.getwidth() > 0) ? 100 : -100, 100);
				}
			}
		}
		// player hurt
		...
		// player title
		// Assetloader.font.draw(batcher, ""+p1.name,
		// p1.getX()+(p1.isleft?p1.getwidth():0), p1.getY()-20);
		...
		for (int i = 0; i < Myworld.enemage.size(); i++) {
			blt = Myworld.enemage.get(i);
			mager = blt.rect;
			if (Myworld.enemage.get(i).getType() == 1) {
				batcher.draw(Assetloader.specialeffA[0].getKeyFrame(runTime), mager.x + (blt.isIsleft() ? mager.width : 0), mager.y, blt.isIsleft() ? -mager.width : mager.width, mager.height);
			} else
				batcher.draw(Assetloader.bulletA.getKeyFrame(runTime), mager.x, mager.y, mager.width, mager.height);
		}
		batcher.setColor(batcherclr.r, batcherclr.g, batcherclr.b, 1f);
		...
		/*
		 * shaper.begin(ShapeType.Filled); shaper.setAutoShapeType(true); shaper.setColor(Color.BLACK); shaper.rect(Myworld.start.x,Myworld.start .y,Myworld.start.width,Myworld.start.height);
		 * shaper.rect(Myworld.end. x,Myworld.end.y,Myworld.end.width,Myworld.end.height); shaper.end();
		 */
		...
		if (Myworld.isCUTSCENE()) {
			...
			// mage-icon
			// batcher.draw(Assetloader.ui[3],Myworld.s[3].x-Myworld.s[3].radius-5,
			// Myworld.s[3].y-Myworld.s[3].radius-5, Myworld.s[3].radius * 2 +
			// 10, Myworld.s[3].radius * 2 + 10);
			...
			// slide-icon
			// batcher.draw(Assetloader.ui[3],Myworld.s[0].x-Myworld.s[0].radius-5,
			// Myworld.s[0].y-Myworld.s[0].radius-5, Myworld.s[0].radius * 2 +
			// 10, Myworld.s[0].radius * 2 + 10);
			...
			// airpunch-icon
			// batcher.draw(Assetloader.ui[3],Myworld.s[1].x-Myworld.s[1].radius-5,
			// Myworld.s[1].y-Myworld.s[1].radius-5, Myworld.s[1].radius * 2 +
			// 10, Myworld.s[1].radius * 2 + 10);
			...
			// hurricane-icon
			// batcher.draw(Assetloader.ui[3],Myworld.s[2].x-Myworld.s[2].radius-5,
			// Myworld.s[2].y-Myworld.s[2].radius-5, Myworld.s[2].radius * 2 +
			// 10, Myworld.s[2].radius * 2 + 10);
			...
			/*
			 * shaper.begin(ShapeType.Line); shaper.setColor(Color.RED); shaper.rect(p1.rect.x, p1.rect.y, p1.rect.width, p1.rect.height); for(Enemy enemy:en) {
			 * shaper.rect(enemy.rect.x,enemy.rect.y,enemy. rect.width,enemy.rect.height); shaper.circle(enemy.getX(),enemy.getY(), 5); }
			 */
			/*
			 * shaper.circle(Myworld.c1.x, Myworld.c1.y, Myworld.c1.radius); shaper.circle(Myworld.c2.x, Myworld.c2.y, Myworld.c2.radius); shaper.circle(Myworld.c3.x, Myworld.c3.y, Myworld.c3.radius);
			 * shaper.circle(Myworld.c4.x, Myworld.c4.y, Myworld.c4.radius); shaper.circle(Myworld.c5.x, Myworld.c5.y, Myworld.c5.radius); shaper.circle(Myworld.c6.x, Myworld.c6.y, Myworld.c6.radius);
			...
			/*
			 * if(Myworld.lvlbtn) { shaper.circle(Myworld.nxtlvl.x, Myworld.nxtlvl.y, Myworld.nxtlvl.radius); //shaper.line(440, 60, 460, 60); //shaper.line(450, 50, 460, 60); //shaper.line(460, 60,
			 * 450, 70); }
			 */
			...
			/*
			 * for(int i=0;i<Myworld.s.length;i++) shaper.circle(Myworld.s[i].x, Myworld.s[i].y, Myworld.s[i].radius); //shaper.circle(p1.getX(),p1.getY(), 5); //stat-bars
			 * shaper.rect(hp.bar.x,hp.bar.y,hp.bar.width,hp.bar.height); shaper.rect(mp.bar.x,mp.bar.y,mp.bar.width,mp.bar.height); shaper.rect(xp.bar.x,xp.bar.y,xp.bar.width,xp.bar.height);
			 */
			shaper.setAutoShapeType(true);
			/*
			 * for (Enemy enemy : en){ shaper.rect(enemy.rect.x, enemy.rect.y, enemy.rect.width, enemy.rect.height); shaper.rect(enemy.innerrect.x, enemy.innerrect.y, enemy.innerrect.width,
			 * enemy.innerrect.height); }
			 */
			...
			// shaper.rect(hp.innerbar.x,hp.innerbar.y,hp.innerbar.width,hp.innerbar.height);
			for (Enemy enemy : en) {
				// shaper.rect(enemy.rect.x, enemy.rect.y, enemy.rect.width,
				// enemy.rect.height);
				if (enemy.isAlive)
					shaper.rect(enemy.getX() - (enemy.isleft ? enemy.width : 0), enemy.getY(), enemy.hpb.innerbar.width, enemy.hpb.innerbar.height);
				// shaper.rect(enemy.basic.rect.x, enemy.basic.rect.y,
				// enemy.basic.rect.width, enemy.basic.rect.height);
			}
			shaper.setColor(Color.BLUE);
			// shaper.rect(mp.innerbar.x,mp.innerbar.y,mp.innerbar.width,mp.innerbar.height);
			for (Enemy enemy : en) {
			...
			/*
			 * shaper.setColor(Color.GREEN); for(int i=0;i<Myworld.mage.size();i++) { mager=Myworld.mage.get(i).rect; shaper.rect(mager.x,mager.y,mager.width,mager.height); }
			 * shaper.setColor(Color.YELLOW); for(int i=0;i<Myworld.enemage.size();i++) { mager=Myworld.enemage.get(i).rect; shaper.rect(mager.x,mager.y,mager.width,mager.height); }
			 */
			...
			/*
			 * if(p1.isAttack) shaper.rect(p1.basic.rect.x, p1.basic.rect.y, p1.basic.rect.width, p1.basic.rect.height);
			 * 
			 * /*if(Myworld.getopponent().isAttack && Myworld.getopponent().skill.delay>0) shaper.rect(Myworld.getopponent().skill.rect.x, Myworld.getopponent().skill.rect.y,
			 * Myworld.getopponent().skill.rect.width, Myworld.getopponent().skill.rect.height);
			 */
			shaper.end();
			// ui end
		}
